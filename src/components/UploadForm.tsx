...
export default function UploadForm(props: UploadFormProps) {
    ...
    const upload = async () => {
        ...
        const promises = files.map(async (file, index) => {
            if (uploadedIndexes.find(i => i === index)) return
    ...
    return (
        <div className="absolute top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] rounded-md w-[50%] h-[600px] bg-white shadow-xl/30 flex flex-col overflow-hidden">
            ...
            <div className="relative h-[80%] p-3" onDrop={onDropHandler} onDragOver={onDragOverHandler} onDragLeave={onDragLeaveHandler}>
                <div style={{borderColor: dragAreaColor}} className={`p-3 w-full h-full border-6 border-dashed select-none rounded-md overflow-hidden`}>
                    ...
                    <div className="w-full h-full grid grid-cols-3 auto-rows-[150px] gap-4 overflow-y-scroll">
                        {files.map((file, index) => {
                            return (
                                <div key={index} className="relative w-full h-full overflow-hidden flex flex-col  border-stone-500 border-1 rounded-sm">
                                    ...
                                    {
                                    uploadedIndexes.find(i => i === index) ? <div className="absolute top-0 left-0 w-full h-full bg-white/80 flex justify-center items-center"><Image src="/done.svg" width={50} height={50} alt=""/></div> : (
                                    errorIndexes.find(i => i === index) ? <div className="absolute top-0 left-0 w-full h-full bg-white/80 flex justify-center items-center"><Image src="/error.svg" width={50} height={50} alt=""/></div> : (
                                    isUploading && <div className="absolute top-0 left-0 w-full h-full bg-white/80 flex justify-center items-center"><Image src="/loading.gif" width={50} height={50} alt=""/></div>
                                    ))}
                                </div>
                            )
                        })}
            ...
            <div className="h-[10%] p-3 w-full flex justify-end">
                <button className="cursor-pointer flex items-center gap-2" onClick={upload}>
                    <Image src="/upload.svg" width={30} height={30} alt="close"/>
                </button>
            </div>
        </div>
    )
